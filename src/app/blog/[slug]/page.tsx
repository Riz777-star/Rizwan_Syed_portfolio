...
export default function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const implementedSlugs = [
        ...
        'face-mask-detection-with-alarm'
    ];
            ...
            <div className="container max-w-7xl mx-auto px-6 mb-16">
                <div className="flex flex-col md:flex-row items-center justify-between border-y border-border/40 py-6 gap-6">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                        ...
                        <Link href="https://linkedin.com/in/rizwan-syed-79b798211" target="_blank" className="p-2 text-muted-foreground hover:text-primary transition-colors bg-secondary/10 rounded-lg hover:bg-primary/10">
                            <Linkedin className="w-4 h-4" />
                        </Link>
                        <Link href="https://www.credly.com/users/rizwan-syed.6b6ea3c9" target="_blank" className="p-2 text-muted-foreground hover:text-primary transition-colors bg-secondary/10 rounded-lg hover:bg-primary/10">
                            <Github className="w-4 h-4" />
                        </Link>
                        <Link href="mailto:syedsrizwansr@gmail.com" target="_blank" className="p-2 text-muted-foreground hover:text-primary transition-colors bg-secondary/10 rounded-lg hover:bg-primary/10">
...
model.save_pretrained_gguf("model_name", tokenizer, quantization_method = "q4_k_m")`}
                            ...
                            {slug === 'face-mask-detection-with-alarm' && (
                                <>
                                    <h2 id="overview" className="text-3xl font-bold mb-6 text-foreground scroll-mt-32">
                                        Published Research
                                    </h2>
                                    <p className="mb-8">
                                        <strong>Face Mask Detection with Alarm using Machine Learning</strong> was
                                        published in the International Research Journal of Modernization in
                                        Engineering, Technology and Science (IRJMETS), Volume 05, Issue 03, March
                                        2023. I developed the project with Ganesh and Velvizhi at Sathyabama
                                        Institute of Science and Technology.
                                    </p>
                                    <p className="mb-8">
                                        The goal was to move beyond a single image classifier and build a practical,
                                        real-time monitoring pipeline. The resulting system checks whether people
                                        are wearing masks, evaluates spacing between detected people, marks
                                        violations on the video feed, and activates an alarm when intervention is
                                        needed.
                                    </p>

                                    <h2 id="architecture" className="text-3xl font-bold mb-6 text-foreground scroll-mt-32">
                                        System Architecture
                                    </h2>
                                    <p className="mb-8">
                                        A camera feed is processed frame by frame with <strong>OpenCV</strong>.
                                        Faces are passed to a MobileNet-based convolutional neural network built
                                        with TensorFlow and Keras, which classifies each face as mask or no mask.
                                        In parallel, <strong>YOLOv3</strong> detects people in the scene.
                                        Euclidean-distance calculations then estimate whether detected people are
                                        maintaining appropriate separation.
                                    </p>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-10">
                                        {[
                                            ['Mask classification', 'MobileNet-based CNN classifies detected faces in real time.'],
                                            ['Person detection', 'YOLOv3 identifies people within each video frame.'],
                                            ['Distance analysis', 'Euclidean distance flags unsafe proximity between detections.'],
                                            ['Alert response', 'Red bounding boxes and an audible alarm identify non-compliance.'],
                                        ].map(([title, description]) => (
                                            <div key={title} className="rounded-2xl border border-border/40 bg-secondary/5 p-6">
                                    ...
                                    <h2 id="contribution" className="text-3xl font-bold mb-6 text-foreground scroll-mt-32">
                                        Engineering Contribution
                                    </h2>
                                    <p className="mb-8">
                                        The key contribution is the integration of two computer-vision tasks into
                                        one operational workflow. Mask classification alone cannot identify
                                        crowding, while person detection alone cannot assess mask compliance. By
                                        combining both, the project produces a more useful safety signal from a
                                        standard live camera feed. MobileNet was selected to keep inference
                                        lightweight enough for real-time use.
                                    </p>
                                    <p className="mb-8">
                                        This work strengthened my practical understanding of dataset preparation,
                                        model integration, video-processing pipelines, object detection, and the
                                        trade-offs required when moving machine-learning models from notebooks
                                        into responsive applications.
                                    </p>

                                    <h2 id="publication" className="text-3xl font-bold mb-6 text-foreground scroll-mt-32">
                                        Read the Paper
                                    </h2>
                                    <p className="mb-8">
                                        The complete paper is available through its DOI and the original IRJMETS
                                        publication archive.
                                    </p>
                                    <div className="flex flex-wrap gap-4 mb-10">
                                        <a
                                            href="https://www.doi.org/10.56726/IRJMETS34706"
                                            target="_blank"
                                        ...
                                        >
                                            Open DOI <LinkIcon className="w-4 h-4" />
                                        </a>
                                        <a
                                            href="https://www.irjmets.com/uploadedfiles/paper/issue_3_march_2023/34706/final/fin_irjmets1679571000.pdf"
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="inline-flex items-center gap-2 rounded-full border border-border px-6 py-3 font-bold text-foreground transition-colors hover:bg-secondary/10"
                                        >
                                            View full paper <BookOpen className="w-4 h-4" />
                                        </a>
                                    </div>
                                </>
                            )}
                            ...
                            <div className="sticky top-12 space-y-8">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        {slug === 'face-mask-detection-with-alarm' && [
                                            { id: 'overview', label: 'Published Research' },
                                            { id: 'architecture', label: 'System Architecture' },
                                            { id: 'contribution', label: 'Engineering Contribution' },
                                            { id: 'publication', label: 'Read the Paper' }
                                        ].map((item) => (
                                            <a key={item.id} href={`#${item.id}`} className="group flex items-center gap-3 text-sm font-medium transition-colors text-muted-foreground hover:text-foreground">
                                                <span className="h-px w-4 bg-transparent group-hover:bg-border transition-all" />
                                                {item.label}
                                ...
                                <div>
                                    ...
                                    <ul className="space-y-6">
                                        {(slug === 'web3-ux-challenges' ? [
                                            ...
                                            {
                                                title: "Published Paper — DOI Record",
                                                time: "Research",
                                                image: "/blog/face-mask-detection.svg",
                                                category: "IRJMETS",
                                                url: "https://www.doi.org/10.56726/IRJMETS34706"
                                            },
                                            {
                                                title: "Full Paper — IRJMETS PDF",
                                                time: "PDF",
                                                image: "/blog/face-mask-detection.svg",
                                                category: "Publication",
                                                url: "https://www.irjmets.com/uploadedfiles/paper/issue_3_march_2023/34706/final/fin_irjmets1679571000.pdf"
                                            }
                                        ] : slug === 'the-architects-manifesto' ? [
                                            {
