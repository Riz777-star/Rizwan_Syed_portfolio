...
const highlightContent = {
    education: {
        ...
        highlight: "That Shape My Practice",
        description: "M.S. in Computer Science from University at Buffalo, layered with CISSP, AWS Solutions Architect, Microsoft Windows Server Hybrid Administrator, and RHCE — an academic base grounded by industry-recognized certifications."
    },
    journey: {
        title: "Six Years of",
        highlight: "Infrastructure",
        description: "From Vaya Group in Hyderabad to University at Buffalo HPC, Holiday Channel, and now Koike Aronson — each role has expanded the surface area of systems, networks, security, and internal tooling I own end-to-end."
    },
    experience: {
        title: "Turning Security Ideas",
        highlight: "Into Systems",
        description: "Vendor-neutral projects spanning adaptive Zero Trust, autonomous cyber defense, immutable recovery, identity governance, and contextual authorization."
    }
};
...
function ExperienceTabSlider({ isLowPowerMode }: { isLowPowerMode: boolean }) {
    ...
    const tabs: TabItem[] = [
        { id: 'education', label: 'Education', description: 'M.S. in Computer Science at University at Buffalo, plus industry certifications (CISSP, AWS SAA, MS Windows Server Hybrid Admin, RHCE).' },
        { id: 'journey', label: 'Journey', description: 'A timeline of infrastructure/systems/security roles across enterprise environments.' },
        { id: 'experience', label: 'Experience', description: 'Detailed breakdown of professional roles, responsibilities, and outcomes.' },
    ];
    ...
    return (
        ...
        >
            <div className="flex items-center gap-4 relative z-10 w-full">
                ...
                {logo && (
                    <div className="shrink-0 w-12 h-12 md:w-16 md:h-16 rounded-lg bg-white border border-neutral-200 overflow-hidden relative shadow-sm">
                        <Image src={logo} alt="Logo" fill className="object-contain p-1" unoptimized loading="lazy" />
                    </div>
                )}
            </div>
                    ...
                    <div key={exp.id} className="relative pl-8 border-l-2 border-neutral-200 dark:border-neutral-800 group/timeline">
                        ...
                        {exp.logo && (
                            <div className="absolute top-0 right-full mr-6 w-32 h-10 md:w-40 md:h-16 opacity-0 group-hover/timeline:opacity-100 transition-all duration-300 pointer-events-none flex items-center justify-end -translate-x-4 group-hover/timeline:translate-x-0 hidden md:flex">
                                <div className="relative w-full h-full rounded-xl bg-white border border-neutral-200 shadow-lg">
                                    <Image 
                                        src={exp.logo} 
