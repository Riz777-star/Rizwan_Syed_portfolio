...
export default function StatsSection({ scrollYProgress, showOnly }: { scrollYProgress?: any, showOnly?: 'top' | 'bottom' }) {
    ...
    // Never show more cards than there are posts, otherwise the wrap-around
    // below repeats the same post and collides on its React key.
    const visibleCount = Math.min(3, blogs.length);

    useEffect(() => {
        const galleryImages = portfolioData.gallery.map(item => ({
            src: item.url,
            alt: item.title,
        }));
