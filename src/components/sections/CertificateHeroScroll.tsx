...
// Certificate scans, drawn from the same source as the /achievements page.
// Absolute paths under /public — createCertItem() below skips prepending
// /certificate/ when the entry already starts with a slash.
const CERTIFICATE_IMAGES = portfolioData.achievements
    .map((achievement) => achievement.image)
    .filter((image): image is string => Boolean(image));

// The hero composition lays out six tiles (cert1..cert6); repeat the available
// scans if there are fewer so no position is left empty.
const CERTIFICATE_POOL = Array.from(
    { length: Math.max(6, CERTIFICATE_IMAGES.length) },
    (_, i) => CERTIFICATE_IMAGES[i % CERTIFICATE_IMAGES.length]
);
...
const CertificateHeroScroll: FC<CertificateHeroScrollProps> = ({ onDownloadClick, isLowPowerMode: isLowPowerModeProp }) => {
    ...
    const createCertItem = useCallback((filename: string): ImageItem => {
        const isAbsolute = filename.startsWith('/') || filename.startsWith('http');
        const src = isAbsolute ? filename : `/certificate/${filename}`;
        return {
            id: filename.replace(/\s+/g, '-').replace(/[\/.]/g, '-').toLowerCase(),
            src,
            alt: filename.replace(/\.(pdf|jpg|jpeg|png|webp|svg)$/i, ''),
            ...
            <div ref={fixedContainerRef} className="fixed inset-0 z-10 h-screen w-full overflow-hidden bg-transparent pointer-events-none">
                ...
                {randomCertificates.map((cert, index) => (
                    <div
                        ...
                        className="absolute w-[10px] h-[10px] opacity-0 overflow-hidden shadow-2xl border border-border/20 bg-white dark:bg-card rounded-lg"
                        style={{
                        ...
                        {cert.isPdf ? (
                            ...
                            <>
                                <Image
                                    ...
                                    // Contain rather than cover so full certificates stay legible.
