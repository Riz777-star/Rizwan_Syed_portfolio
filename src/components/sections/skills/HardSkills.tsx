...
export const HardSkills = () => {
  ...
  const categorizedSkills = useMemo(() => {
    ...
    portfolioData.hardSkills.forEach(skill => {
      ...
      } else if (skill.category === 'cloud' || skill.category === 'devops') {
        groups.infrastructure.push(skill);
      } else {
        groups.automation.push(skill);
      }
    });
    return groups;
  }, []);
      ...
      <div className="w-full max-w-[1400px] px-4 md:px-8 mx-auto relative z-10 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 w-full relative z-10 items-stretch">
          ...
          >
            ...
            <div
              data-lenis-prevent={expanded.infrastructure ? "true" : undefined}
              onWheel={expanded.infrastructure ? (e) => e.stopPropagation() : undefined}
              className={cn(
                "flex flex-col gap-4 flex-grow min-h-0 transition-all duration-300 pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-black/10 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full",
                expanded.infrastructure ? "overflow-y-auto" : "overflow-hidden"
              )}
            >
              <AnimatePresence>
                {categorizedSkills.infrastructure.slice(0, expanded.infrastructure ? undefined : 3).map((skill, idx) => (
                  <SkillCard key={skill.name} skill={skill} delay={idx * 0.05} />
                ))}
              </AnimatePresence>
            </div>
            {categorizedSkills.infrastructure.length > 3 && (
              <div className="shrink-0 mt-auto pt-4">
                <button
                ...
                >
                  {expanded.infrastructure ? (
                    ...
                    <>View More ({categorizedSkills.infrastructure.length - 3}) <ChevronDown className="w-4 h-4" /></>
                  )}
                </button>
              </div>
            )}
          ...
          >
            ...
            <div
              data-lenis-prevent={expanded.automation ? "true" : undefined}
              onWheel={expanded.automation ? (e) => e.stopPropagation() : undefined}
              className={cn(
                "flex flex-col gap-4 flex-grow min-h-0 transition-all duration-300 pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-black/10 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full",
                expanded.automation ? "overflow-y-auto" : "overflow-hidden"
              )}
            >
              <AnimatePresence>
                {categorizedSkills.automation.slice(0, expanded.automation ? undefined : 3).map((skill, idx) => (
                  <SkillCard key={skill.name} skill={skill} delay={idx * 0.05} />
                ))}
              </AnimatePresence>
            </div>
            {categorizedSkills.automation.length > 3 && (
              <div className="shrink-0 mt-auto pt-4">
                <button
                ...
                >
                  {expanded.automation ? (
                    ...
                    <>View More ({categorizedSkills.automation.length - 3}) <ChevronDown className="w-4 h-4" /></>
                  )}
                </button>
              </div>
            )}
      ...
      <div className="mb-4 relative z-10">
        <div className="flex justify-between items-start mb-4 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            {skill.icon && (
              <div className="relative w-11 h-11 shrink-0 rounded-xl bg-black/[0.03] dark:bg-white/[0.06] border border-black/5 dark:border-white/10">
                <Image src={skill.icon} alt="" fill sizes="44px" className="object-contain p-2.5" />
              </div>
            )}
