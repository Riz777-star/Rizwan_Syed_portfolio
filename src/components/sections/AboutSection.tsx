...
// Kaggle / WakaTime live showcases stay unrendered until Rizwan provides
// handles for those platforms. Components are kept in the repo for a fast
// re-enable once the data is available.

// Timeline of Rizwan's professional experience (used by HorizontalTimeline),
// newest role first. Falls back to the profile photo for any role without a
// company logo.
const FALLBACK_IMAGE = portfolioData.personal.avatar;

const TIMELINE_PERIODS: Record<string, string> = {
    'exp-koike': "May 2025 - Present",
    'exp-holiday': "Oct 2024 - May 2025",
    'exp-ub': "Jan 2024 - Sept 2024",
    'exp-vaya': "Jun 2022 - May 2023",
};

const showcaseMembers = [
    ...Object.keys(TIMELINE_PERIODS).flatMap(id =>
        portfolioData.experiences
            ...
            .map(exp => ({
                ...
                period: TIMELINE_PERIODS[exp.id],
                image: exp.logo ?? FALLBACK_IMAGE,
                social: exp.externalLink
                    ? { website: Array.isArray(exp.externalLink) ? exp.externalLink[0] : exp.externalLink }
                    : undefined
            }))
    ),
    {
...
const GALLERY_IMAGES = portfolioData.gallery.map(item => item.url);
            ...
            <div className="relative pointer-events-none mt-[20vh] md:mt-[20vh]">
                ...
                <div className="bg-background dark:bg-black transition-colors duration-500 pointer-events-auto relative">
                    ...
                    <div className="flex flex-col items-center w-full bg-background relative z-20 pt-32 pb-32">
                            ...
                            <div className="w-full pb-0">
                                <HorizontalTimeline data={showcaseMembers.map((member) => ({
                                    ...
                                    content: member.id === 'view-more' ? (
                                        ...
                                        <div className="flex flex-col gap-4 w-[340px] md:w-[460px] border border-neutral-200 dark:border-neutral-800 p-7 rounded-2xl bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md shadow-xl mt-4">
                                            <div className="flex flex-col gap-1">
                                                <h4 className="text-xl font-bold text-neutral-900 dark:text-white leading-tight">
                                                    {member.name}
                                                </h4>
                                                {'role' in member && member.role && (
                                                    <span className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">
                                                        {member.role}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            {/* Logos are artwork, not photos: contain them on a white plate so
                                                wide wordmarks aren't cropped and dark ink stays readable. */}
                                            <div className="w-full mt-1 rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden relative group/card h-24 bg-white">
                                                <img
                                                    src={member.image}
                                                    alt={member.name}
                                                    className="w-full h-full object-contain p-5 transition-transform duration-500 group-hover/card:scale-105"
                                                />
                        ...
                        {/*
                            Kaggle / WakaTime showcases hidden until Rizwan provides
                            handles for those platforms.

                            <div className="w-full"><KaggleShowcase /></div>
                            <div className="w-full"><WakaTimeShowcase /></div>
                        */}
                    </div>
                    <AuditFunnel />
                </div>
