...
// Brands with a logo asset render as an image; the rest fall back to a text
// badge so the marquee stays consistent as more logos are added.
const GalleryItem = ({ brand }: { brand: MarqueeBrand }) => {
    return (
        <div className="relative shrink-0 w-[clamp(140px,30vw,200px)] h-[clamp(80px,15vw,120px)] md:w-[280px] md:h-[160px] flex items-center justify-center group cursor-pointer transition-all duration-300 hover:scale-105">
            {brand.logo ? (
                ...
                <div className="text-center px-4">
                    <span className="font-black uppercase tracking-tight text-xl md:text-3xl text-neutral-500 dark:text-neutral-400 group-hover:text-foreground opacity-70 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap">
                        {brand.label}
                    </span>
                </div>
            )}
...
export default function ExperienceMarquee() {
    ...
    // Employers, technology stack, and credentials as scrolling brand
    // affiliations. Employer logos lead each row; the rest are text badges.
    const allLogos: MarqueeBrand[] = [
        { label: "Koike Aronson", logo: "/logos/koike.png" },
        { label: "University at Buffalo", logo: "/logos/university-at-buffalo.jpg" },
        { label: "Holiday Channel", logo: "/logos/holiday-channel.png" },
        { label: "Vaya Group", logo: "/logos/vaya-group.jpg" },
        { label: "Microsoft" },
        { label: "AWS" },
        { label: "VMware" },
        { label: "Veeam" },
    ...
    const ensureLength = (items: MarqueeBrand[]) => {
        let repeated = [...items];
        ...
        <section className="py-2 md:py-8 bg-background relative z-10 overflow-hidden">
            ...
            <div className="flex flex-col gap-2">
                ...
                <ParallaxText baseVelocity={40} isLowPowerMode={isLowPowerMode}>
                    {ensureLength(row1).map((brand, idx) => (
                        <GalleryItem key={`r1-${idx}`} brand={brand} />
                    ))}
                </ParallaxText>

                {/* Row 2: RIGHT → LEFT */}
                <ParallaxText baseVelocity={-40} isLowPowerMode={isLowPowerMode}>
                    {ensureLength(row2).map((brand, idx) => (
                        <GalleryItem key={`r2-${idx}`} brand={brand} />
                    ))}
                </ParallaxText>
            </div>
        </section>
