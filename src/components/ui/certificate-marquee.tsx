...
// The three parallax columns below need 10 tiles between them. Cycle through
// whichever achievements actually have a scan attached so the gallery stays
// full as certificates are added or removed.
...
const certificates = Array.from(
  { length: 10 },
  (_, i) => certificateImages[i % certificateImages.length]
);
...
const Column = ({ images, y }: ColumnProps) => {
  return (
    ...
    >
      {images.map((src, i) => (
        <div key={i} className="relative w-full overflow-hidden rounded-none bg-white dark:bg-zinc-900 ring-1 ring-black/5 dark:ring-white/10" style={{ paddingTop: '75%' }}>
          <Image
            src={src}
            alt={`Certificate ${i}`}
            fill
            sizes="(max-width: 1024px) 50vw, 33vw"
            // Certificates vary from wide letterheads to tall A4 scans; contain
            // keeps the whole document readable instead of cropping it.
          ...
          <div className="space-y-6 w-full">
            <div className="space-y-4">
              ...
              <p className="text-lg text-muted-foreground max-w-none leading-relaxed lg:whitespace-nowrap">
                CISSP, AWS Solutions Architect, Microsoft Windows Server Hybrid Admin, RHCE — plus academic and specialist credentials.
              </p>
            </div>
          </div>
